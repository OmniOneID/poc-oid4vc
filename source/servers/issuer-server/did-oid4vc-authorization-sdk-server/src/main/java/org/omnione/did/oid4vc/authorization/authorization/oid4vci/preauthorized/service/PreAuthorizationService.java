...
package org.omnione.did.oid4vc.authorization.authorization.oid4vci.preauthorized.service;

import org.omnione.did.oid4vc.authorization.authorization.oid4vci.preauthorized.dto.PreAuthorizedCode;
import org.omnione.did.oid4vc.authorization.authorization.repository.InMemoryPreAuthorizedCodeRepository;
...
public class PreAuthorizationService {
    ...
    public PreAuthorizedCode create(String userId, Set<String> scopes, String cNonce) {
        String code = UUID.randomUUID().toString();
        String userPin = generateUserPin();
        Instant expiresAt = Instant.now().plusSeconds(PRE_AUTHORIZED_CODE_EXPIRY_SECONDS);
        PreAuthorizedCode preAuthorizedCode = new PreAuthorizedCode(code, userId, expiresAt, scopes, userPin, cNonce);
