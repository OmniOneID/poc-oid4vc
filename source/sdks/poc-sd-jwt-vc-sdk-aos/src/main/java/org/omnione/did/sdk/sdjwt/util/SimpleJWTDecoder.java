...
public class SimpleJWTDecoder {
  ...
  public static SimpleJWT parse(String jwtString) {
    if (jwtString == null || jwtString.trim().isEmpty()) {
      throw new SDJWTException("JWT string cannot be null or empty");
    }

    String[] parts = jwtString.split("\\.");
    if (parts.length != 3) {
      throw new SDJWTException("Invalid JWT format: expected 3 parts separated by dots");
    }
  ...
  public static boolean isJWTFormat(String input) {
    ...
    try {
      ...
      throw new SDJWTException("Failed to parse JWT header: " + e.getMessage(), e);
    } catch (JsonProcessingException e) {
      throw new SDJWTException("Failed to parse JWT header: " + e.getMessage(), e);
    }
  }
