...
public class ECDSAVerifier implements JWSVerifier {
  ...
  public boolean verify(SignedJWT signedJWT) throws SDJWTException {
    try {
      ...
      throw new SDJWTException("ECDSA signature verification failed: " + e.getMessage(), e);
    } catch (InvalidKeyException e) {
      throw new SDJWTException("Invalid EC public key: " + e.getMessage(), e);
  ...
  private byte[] convertP1363ToDER(byte[] p1363Signature) {
    if (p1363Signature.length != 64) {
      throw new SDJWTException("IEEE P1363 signature must be 64 bytes");
    }
