...
package org.omnione.did.sdk.sdjwt.core.validator;
...
public class SDJWTValidator {
  ...
  private void validateCredentialJWT(String credentialJwt, List<String> errors,
    ...
    try {
      ...
      if (header.has("typ")) {
        ...
        if (!"dc+sd-jwt".equals(typ) && !"JWT".equals(typ)) {
          warnings.add("Credential JWT type is not 'dc+sd-jwt' or 'JWT': " + typ);
        }
      }
      ...
      throw new SDJWTException("Failed to parse credential JWT header: " + e.getMessage(), e);
  ...
  private void crossValidateDisclosures(SDJWT sdjwt, List<String> errors, List<String> warnings,
    ...
    try {
      ...
      throw new SDJWTException("Failed to cross-validate disclosures: " + e.getMessage(), e);
    } catch (JsonProcessingException e) {
      throw new SDJWTException("Failed to cross-validate disclosures: " + e.getMessage(), e);
    }
  }
